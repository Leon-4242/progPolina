...
// Расчет бесконечной нормы для производной функции
double compute_derivative_infinity_norm (
    const abstract_function_1d *func, double a, double b, int n_steps)
{
  double len = b - a;
  double norm = fabs (func->derivative (b));
  double step = len / static_cast<double> (n_steps);
  for (int i = 0; i < n_steps; i++)
    {
      double y = fabs (func->derivative (a + i * step));
      if (y > norm)
        norm = y;
    }
  return norm;
...
double function_1d::derivative (double x) const
{
  switch (m_k)
  {
  case function_id_t::constant:       
...
double abstract_interpolation_function_1d::derivative (double x) const
{
  return compute_interpolated_derivative (x, m_a, m_b, m_n, m_points, m_coeffs);
}
    ...
    int n, double a, double b, int p, double norm, const abstract_function_1d *func)
{
  ...
  m_coeffs = allocate_coefficients (2 * n);
  ...
  for (int i = 0; i < n; i++)
    {
      ...
      values[i + n] = func->derivative (m_points[i]);
      if (i == permutated_index)
        {
          values[i] += p * 0.1 * m_norm;
          values[i + n] += p * 0.1 * m_norm;
        }
    }
...
double residual_function_1d::derivative (double x) const
{
  return m_func->derivative (x) - m_approx->derivative (x);
}