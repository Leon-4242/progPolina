...
QString draw_mode_to_string (draw_mode_t draw_mode)
{
  ...
  case draw_mode_t::function_1:
    return QString::fromUtf8 ("Method 1");
  case draw_mode_t::function_2:
    return QString::fromUtf8 ("Method 2");
  case draw_mode_t::function_12:
    return QString::fromUtf8 ("Both Methods");
...
graph_widget_t::graph_widget_t (QWidget *parent, double a, double b, int n, int k)
    : QWidget (parent),
      ...
      m_Newton_linear_residual (&m_func, &m_Newton_linear_interpolation)
{
  m_a = a;
  m_b = b;
  m_n = n;
...
{
  m_Newton_linear_interpolation.set_points (m_n, m_a, m_b, m_p, m_func_infinity_norm, &m_func);
  ...
  for (int pixel = pixel_min; pixel <= pixel_max; pixel++)
    {
      ...
      switch (m_draw_mode)
        ...
        case draw_mode_t::residual:
          ...
          update_min_max_by_function_value (&m_Newton_linear_residual, x, m_min_y, m_max_y, first_step);
          break;
        case draw_mode_t::COUNT:
          assert (false);
          break;
...
void graph_widget_t::draw_residuals (QPainter &painter)
{
  QPen pen_blue(Qt::blue, 0, Qt::SolidLine);
  painter.setPen (pen_blue);
  draw_graph (painter, &m_Newton_linear_residual);
